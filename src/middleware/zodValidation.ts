    ...
    try {
      ...
      if (error instanceof z.ZodError) {
        ...
        return res.status(400).json({
          ...
          summary: formattedErrors.map(err => err.message)
        });
      }
    ...
    try {
      ...
      if (errors.length > 0) {
        return res.status(400).json({
          error: "Datos de entrada inválidos",
          details: errors,
          summary: errors.map(err => `${err.target}.${err.field}: ${err.message}`)
        });
      }
...
export const basicSanitization = (req: Request, res: Response, next: NextFunction) => {
