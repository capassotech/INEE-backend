...
export const createPreference = async (req: Request, res: Response) => {
  ...
  export const handleWebhook = async (req: Request, res: Response) => {
      try {
          ...
          // Verificar si ya se procesó un webhook con el mismo estado
          // Esto evita enviar correos duplicados cuando MercadoPago envía múltiples notificaciones
          const isStatusChange = previousStatus !== payment.status;
          
          if (!isStatusChange && previousStatus === 'approved') {
              console.log(`ℹ️  Webhook duplicado detectado - El pago ${paymentId} ya fue procesado como 'approved'`);
              return res.sendStatus(200);
          }
  
          // Usar una transacción para evitar race conditions cuando múltiples webhooks
          ...
          try {
              await firestore.runTransaction(async (transaction) => {
                  ...
                  if (!orderSnapshot.exists) {
                      throw new Error('Orden no encontrada en transacción');
                  }
                  
                  const currentOrderData = orderSnapshot.data();
                  const currentPaymentStatus = currentOrderData?.paymentStatus;
                  
                  // Verificar nuevamente dentro de la transacción para evitar race conditions
                  if (currentPaymentStatus === 'approved' && payment.status === 'approved') {
                      console.log(`ℹ️  Webhook duplicado detectado en transacción - El pago ${paymentId} ya fue procesado`);
              ...
              if (transactionError.message === 'WEBHOOK_ALREADY_PROCESSED') {
                  console.log(`✅ Webhook duplicado ignorado correctamente`);
                  return res.sendStatus(200);
              }
